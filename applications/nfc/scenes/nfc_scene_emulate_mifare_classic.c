...
void nfc_emulate_mifare_classic_worker_callback(void* context) {
    Nfc* nfc = (Nfc*)context;
    scene_manager_set_scene_state(
        nfc->scene_manager, NfcSceneEmulateMifareClassic, NFC_MF_UL_DATA_CHANGED);
}
...
void nfc_scene_emulate_mifare_classic_on_enter(void* context) {
    ...
    popup_set_header(popup, "Emulating\nMifare Classic", 56, 31, AlignLeft, AlignTop);
    ...
    if(event.type == SceneManagerEventTypeTick) {
        ...
        if(scene_manager_get_scene_state(nfc->scene_manager, NfcSceneEmulateMifareClassic) ==
           NFC_MF_UL_DATA_CHANGED) {
            scene_manager_set_scene_state(
                nfc->scene_manager, NfcSceneEmulateMifareClassic, NFC_MF_UL_DATA_NOT_CHANGED);
