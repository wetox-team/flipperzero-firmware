...
void nfc_device_prepare_format_string(NfcDevice* dev, string_t format_string) {
    if(dev->format == NfcDeviceSaveFormatUid) {
        ...
        string_set_str(
            format_string, nfc_mf_classic_type(dev->dev_data.mf_classic_data.type, true));
    } else {
...
bool nfc_device_parse_format_string(NfcDevice* dev, string_t format_string) {
    ...
    // Check Mifare Classic types
    for(MfClassicType type = MfClassicTypeUnknown; type < MfClassicTypeNum; type++) {
        if(string_start_with_str_p(format_string, nfc_mf_classic_type(type, true))) {
...
static bool nfc_device_save_mifare_classic_data(FlipperFormat* file, NfcDevice* dev) {
    ...
    do {
        ...
        // Write sectors data
        uint32_t sectors_total = data->data_size / 64;
        if(!flipper_format_write_uint32(file, "Sectors total", &sectors_total, 1)) break;
        bool sectors_saved = true;
        for(uint16_t i = 0; i < data->data_size; i += 64) {
            string_printf(temp_str, "Sector %d", i / 64);
            if(!flipper_format_write_hex(file, string_get_cstr(temp_str), &data->data[i], 64)) {
                sectors_saved = false;
                break;
            }
        }
        if(!sectors_saved) break;
        saved = true;
    } while(false);
...
bool nfc_device_load_mifare_classic_data(FlipperFormat* file, NfcDevice* dev) {
    bool read = false;
    MifareClassicData* data = &dev->dev_data.mf_classic_data;
    string_t temp_str;
    string_init(temp_str);
    ...
    do {
        // Read sectors
        uint32_t sectors = 0;
        if(!flipper_format_read_uint32(file, "Sectors total", &sectors, 1)) break;
        data->data_size = sectors * 64;
        bool sectors_read = true;
        for(uint16_t i = 0; i < sectors; i++) {
            string_printf(temp_str, "Sector %d", i);
            if(!flipper_format_read_hex(file, string_get_cstr(temp_str), &data->data[i * 64], 64)) {
                sectors_read = false;
                break;
            }
        }
        if(!sectors_read) break;
        read = true;
    } while(false);
